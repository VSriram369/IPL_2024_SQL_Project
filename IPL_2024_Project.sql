...
venue_area VARCHAR(50) NOT NULL,
...
INSERT INTO team VALUES(1,'Royal Challengers Bangalore');
...
FOREIGN KEY (dismissed_batman_id) REFERENCES player(player_id)
);
...
-- ALTER TABLE score_by_ball
-- RENAME COLUMN runs_of_bat TO runs_off_bat;

-- ALTER TABLE score_by_ball
-- RENAME COLUMN penality TO penalty;
...
/*
SQL statements required to populate the 
...
INNER JOIN score_by_ball s 
ON (i.match_id = s.match_id AND i.innings_no = s.innings_no)
GROUP BY i.match_id,
i.innings_no,
t.team_name;
...
SELECT 
    ...
    CASE 
        WHEN MOD(COUNT(*) - COUNT(s.wides) - COUNT(s.no_balls), 6) = 0
        THEN (COUNT(*) - COUNT(s.wides) - COUNT(s.no_balls)) / 6
        ELSE CONCAT(
            FLOOR((COUNT(*) - COUNT(s.wides) - COUNT(s.no_balls)) / 6),
            '.',
            MOD((COUNT(*) - COUNT(s.wides) - COUNT(s.no_balls)), 6)
        )
    END AS total_overs
FROM 
    innings i 
...
SELECT 
    ...
    SUM(CASE WHEN s.runs_off_bat = 4 THEN 1 ELSE 0 END) AS no_of_fours,
    SUM(CASE WHEN s.runs_off_bat = 6 THEN 1 ELSE 0 END) AS no_of_sixes,
    FORMAT((SUM(s.runs_off_bat) / COUNT(*)) * 100, 2) AS strike_rate
FROM 
    innings i 
    JOIN team t ON i.batting_team_id = t.team_id
    JOIN score_by_ball s ON i.match_id = s.match_id AND i.innings_no = s.innings_no
...
WHERE 
    s.wides IS NULL AND s.no_balls IS NULL
GROUP BY 
    i.match_id,
    i.innings_no,
    t.team_name,
...
SELECT i.match_id,
            ...
            SUM(IFNULL(s.no_balls, 0)) AS total_noballs,
            SUM(IFNULL(s.wides, 0)) AS total_wides,
            SUM(IFNULL(s.byes, 0)) AS total_byes,
            SUM(IFNULL(s.leg_byes, 0)) AS total_legbyes,
...
SELECT 
    ...
    END AS total_overs,
    SUM(s.runs_off_bat + IFNULL(s.wides, 0) + IFNULL(s.no_balls, 0)) AS total_runs,
    COUNT(s.wicket_type) AS total_wickets,
    ROUND(
        SUM(s.runs_off_bat + IFNULL(s.wides, 0) + IFNULL(s.no_balls, 0)) / (COUNT(*) - COUNT(s.wides) - COUNT(s.no_balls)) * 6, 
        2
    ) AS economy,
    SUM(CASE WHEN s.runs_off_bat = 0 AND s.wides IS NULL AND s.no_balls IS NULL THEN 1 ELSE 0 END) AS dots
FROM 
    innings i 
    JOIN team t ON i.batting_team_id = t.team_id
    JOIN score_by_ball s ON i.match_id = s.match_id AND i.innings_no = s.innings_no
